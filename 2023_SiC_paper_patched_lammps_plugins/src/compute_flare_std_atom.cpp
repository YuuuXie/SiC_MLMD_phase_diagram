...
void ComputeFlareStdAtom::compute_peratom() {
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // Variance from the sparse GP: v_T beta v / |v|^2 (skip empty environments).
    if (B2_norm_squared > 1e-12) {
      Eigen::VectorXd beta_p = beta_matrices[itype - 1] * B2_vals;
      variance = B2_vals.dot(beta_p) / B2_norm_squared;
    } else {
      variance = 0.0;
    }
