...
void B1_descriptor(Eigen::VectorXd &B1_vals, Eigen::MatrixXd &B1_env_dervs,
  ...
  // B1 is the rotation-invariant l=0, m=0 component of the single-bond basis.
  // For each (species s, radial n) pair we take A_{s,n,l=0,m=0}.
  int env_derv_size = single_bond_env_dervs.rows();
  int neigh_size = env_derv_size / 3;
  int n_radial = n_species * N;
  int n_harmonics = (lmax + 1) * (lmax + 1);
  int n_descriptors = n_radial;
  ...
  // Copy the (l=0, m=0) slice from the single-bond representation.
  for (int n = 0; n < n_radial; n++) {
    int idx = n * n_harmonics; // l=0, m=0 sits at the start of each radial block
    B1_vals(n) = single_bond_vals(idx);

    for (int atom_index = 0; atom_index < neigh_size; atom_index++) {
      for (int comp = 0; comp < 3; comp++) {
        B1_env_dervs(atom_index * 3 + comp, n) =
            single_bond_env_dervs(atom_index * 3 + comp, idx);
      }
    }
  }
